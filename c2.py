...
class B1_Wander_Test:
    def __init__(self):
        ...
        # initialize booleans for handling bumps and avoiding obstacles
        self.crbump = False
        self.lbump = False
    ...
    def positionToMap(self, position):
        ...
        # convert distance from orignial position to map units
        step_x = int(position[0]/world_map_ratio)
        step_y = int(position[1]/world_map_ratio)
        
        # need to account for the fact that the robot does not start at (0,0) on the map 
        return (step_x + 2, step_y + 15)


    def initializeMap(self):
        # first map update, need to do twice because it doesn't show up nicely the first time 
    ...
    def updateMapFree(self):
        ...
        current_pos_map = self.positionToMap(self.position)
        ...
        if (current_pos_map[0] <= 40 and current_pos_map[0] >= 0 and current_pos_map[1] <= 30 and current_pos_map[1] >= 0):
                # if the current position is ok, set it to be free, as well as the surrounding pixels 
    ...
    def updateMapOccupied(self):
        ...
        """
        # current position and obstacle position on the map 
        current_pos_map = self.positionToMap(self.position)
        obstacle_pos_map = self.positionToMap(self.obstacle_pos)
        ...
        if (current_pos_map[0] <= 40 and current_pos_map[0] >= 0 and current_pos_map[1] <= 30 and current_pos_map[1] >= 0):
                ...
                # show the map for a short time, but still relative to current position 
                self.mapObj.UpdateMapDisplay(self.my_map, current_pos)
